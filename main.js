...
const chartG = svg
  ...
  .attr(
    "transform",
    `translate(${margin.left + length / 2},${margin.top + length / 2})`
  );
...
const legendG = svg
  ...
  .attr(
    "transform",
    `translate(${length/2-60}, ${length/2-20})`
  ); 
//   lazy to calculate so the centering is manual
// add all units & text
chartG
...
const legend = legendG
  ...
  .attr(
    "transform",
    (d, i) => `translate(${margin.left},${i * categoryHeight})`
  );
